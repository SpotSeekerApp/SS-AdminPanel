...
def list_places_page():
    ...
    if request.method == "GET":
        ...
        for place_id in place_dict.keys():
            ...
            response = requests.get(f'{API_URL}/GetTagByPlace?place_id={place_id}')
            ...
            if status == HTTPStatus.OK:
                ...
                if tags == None:
                    ...
                    place_dict[place_id]["tags"] = ",".join(tags.keys())
            else:
                msg = f"Error failed to list places {HTTPStatus.INTERNAL_SERVER_ERROR}"
                logger.error(msg)
                place_dict[place_id]["tags"] = ""
...
def update_places_page():
    ...
    if request.method == "POST":
        ...
        all_tags = requests.get(f'{API_URL}/GetAllTags').json()["Data"]["all_tags"]
        tag_dict = {}
        for tag in all_tags:
            tag_dict[tag] = 0.0
